...
# --- Get Node Exporter Targets from User ---
read -p "Enter Node Exporter targets (comma-separated IPs: e.g., 192.168.1.10:9100,192.168.1.11:9100): " TARGETS

# Convert the comma-separated input into YAML format
TARGETS_YAML=""
IFS=',' read -ra ADDR <<< "$TARGETS"
for IP in "${ADDR[@]}"; do
    TARGETS_YAML+="      - targets: ['$IP']\n"
done
